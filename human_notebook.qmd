...
chart(biometry, masse_corr ~ taille %col=% genre)+
  geom_point()+
  labs(title = "Représentation de la masse des individus analysés en fonction de leur taille", y= "Masse(kg)" , x= "Taille(cm)" )
```
...
biometry %>.%
  ...
  ssummarise(.,
    ...
    "moyenne de l'IMC" = fmean(masse / (taille / 100)^2),
    "n" = fn(masse_corr)) ->
  Moyenne_tab
...
La formule de la dette de sommeil est:

$\text{dette de sommeil} = \text{sommeil optimal} - \text{sommeil réel}$

Les données ont été trouvées sur le site: https://www.sommeil.org/comprendre-le-sommeil/combien-de-temps-et-d-heures-doit-on-dormir-pour-etre-en-bonne-sante-et-en-forme/
...
tabularise(imc_tabcontingence)#pour vérifier le nombre d'individus par catégorie
...
```{r test_chi2_2}
#Vérification des variables (il nous faut des qualitatives)
#Vérification du nombre total d'individus et l'indépendance des variables
...
Nous observons que la taille des hommes diffère de manière significative au seuil alpha de 5% ( test t bilatéral, t = 12.366, ddl = 138, valeur p = 2.2e-16)
...
```{r test_anova1_1}
bartlett.test(data = biometry, Indice_de_masse_grasse ~ group)
