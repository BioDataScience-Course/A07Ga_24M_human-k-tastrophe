...
```{r}
chart(data = biometry, masse_corr ~ genre %col=% genre %fill=% sucre)+
...
# Calcul de la ou des variables reprenant l'indice sous forme numérique (et facteur)
#biometry %>.%
 # sselect(., alcool, masse, sexe, age) %>.%
  #smutate(., 
  #"Consommation d’alcool occasionnelle" = case_when( 
#    sex == ("F" & "M"), alcool>= 12*12 & alcool <= 12/4 ) , )
biometry %>%
  select(alcool, masse, genre, age) %>%
  smutate(
Consommation_d_alcool_occasionnelle = case_when(
       alcool  >= 12 ~ "Oui",TRUE ~ "Non"))

ifelse(genre =="H" smutate(Consommation_d_alcool_modérée 
      alcool >= 12 & alcool <= 28 ~ 
```
