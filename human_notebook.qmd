...
biometry <- biometry %>%
  smutate(
    ...
    Consommation_risque = case_when(
      ...
      TRUE ~ "Non" ),
    Consommation_alcool = case_when(alcool >= 1 ~ "Oui",
      TRUE ~ "Non")
  ) -> biometry
...
FF<- biometry %>.% 
 sgroup_by(., genre) %>.%
  ssummarise(.,
    "Conso_occasionnelle" = fn(Consommation_occasionnelle),
    "Conso_modérée" = fn(Consommation_modérée),
    "Conso_risque" = fn(Consommation_risque),
    "Conso_alcool" = fn(Consommation_alcool),
    "N" = fn(genre)
    )
tabularise(FF)

#biometry2 <- biometry %>% #
 # sfilter(age >= 18 & age <= 26) %>% 
 # sselect(genre, masse, age, Consommation_modérée, Consommation_occasionnelle, Consomation_alcool )  %>% 
#arrange(genre, age)
#tabularise(biometry2)
```
...
chart(data= biometry, dette_sommeil ~ IMC %shape=% group_age %col=% group_age)+
  geom_point()
...
#Pour utiliser un test t de student, il faut que la variance entre les deux groupes considérés soit similaire.
biometry %>.%
  sgroup_by(., genre) %>.% 
  ssummarise(.,
