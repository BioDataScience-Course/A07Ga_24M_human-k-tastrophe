...
```{r}
chart(data = sdrop_na(biometry,sucre),~alimentation %fill=% sucre)+
geom_bar()+
  xlab("Alimentation") +
  ylab("Effectifs")
```
...
```{r}
chart(dt, masse ~ tour_hanche %col=% genre) +
geom_point()
```

```{r}
chart(dt, ~ activite_physique %fill=% eau) +
geom_histogram()
```
...
```{r}
chart(data = dt, masse ~ genre  %fill=% sucre)+
  geom_violin()
```

```{r}
chart(data = dt, taille ~ genre %fill=% genre |alimentation )+
  geom_boxplot()
```
