...
```{r plot_anova2_1}
anova22<-anova(lm(data=biometry,BMI~genre*activité)%>.% 
    tabularise(.)
```
...
```{r test_anova2_1}
chart$qqplot(anova22)

bartlett.test(data=anova22,BMI ~ interaction(genre, activité))

sselect(biometry,BMI,genre,activité)%>.%
  tabularise(.)
```
