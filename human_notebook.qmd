...
biometry1$sucre<- as.factor(biometry1$group_age)
biometry1$RTH_group_age <- interaction(biometry1$RTH, biometry1$group_age)

anova(RTH_anova2 <- lm(data = biometry1, masse_corr ~ RTH_group_age)) %>.%
tabularise(.)
```
...
sgroup_by(biometry1, RTH, group_age)
ssummarise(biometry1, masse_corr_moyenne = mean(masse_corr))

chart(biometry1, masse_corr_moyenne %col=% RTH %group=% group_age)
+ geom_line()
+ geom_point()

#Vérification des variables et données
tabularise$headtail(sselect(biometry1, masse_corr, group_age, RTH))

biometry1 %>.%
sgroup_by(., RTH, group_age) %>.%
ssummarise(.,
...
bartlett.test(data = biometry1, masse_corr ~ interaction(RTH, group_age))

#Test Posthoc

summary(RTH_testposthoc <- confint(multcomp::glht(RTH_anova2,
  linfct = multcomp::mcp(RTH_group_age = "Tukey"))))
oma <- par(oma = c(0, 5.1, 0, 0))
